...
app.get("/", (request, response) => {
  console.log("Transform pinglendi.");
...
client.on('guildCreate', async(i) => {
let users = i.guild.memberCount
if(users < 20){
i.guild.leave()
let Owner = i.guild.owner
let bye = client.users.cache.get(Owner)
bye.send('Sunucuna eklendim, fakat sunucun 20 kişi altı onay alana kadar 20 kişi altı sunuculardan çıkış yapıyorum') 
}else{
return 
} 
})
    ...
    if (i == 'acik') {
      if (msg.content.toLowerCase() == 'sa' || msg.content.toLowerCase() == 's.a' || msg.content.toLowerCase() == 'selamun aleyküm' || msg.content.toLowerCase() == 'sea'|| msg.content.toLowerCase() == 'selam') {
          try {
                  ...
                  return msg.reply(
                    'Aleyküm Selam, Hoşgeldin Nasılsın')
          } catch(err) {
            console.log(err);
          }
      }
...
const embed = new Discord.MessageEmbed() .setColor('#05c7ff') .setDescription(` ${nev.author} , **Mesajını editleyerek reklam yapmaya çalıştı!**`)
      .addField("Mesajı:",nev)
  
      nev.delete();
      const embeds = new Discord.MessageEmbed() .setColor('#05c7ff') .setDescription(` ${nev.author} , **Mesajı editleyerek reklam yapamana izin veremem!**`) 
...
msg.delete({timeout:750});
              const embeds = new Discord.MessageEmbed() .setColor('#05c7ff') .setDescription(` <@${msg.author.id}> , **Bu sunucuda reklam yapmak yasak!**`)
msg.channel.send(embeds).then(msg => msg.delete({timeout: 5000}));
          const embed = new Discord.MessageEmbed() .setColor('#05c7ff') .setDescription(` ${msg.author} , **Reklam yapmaya çalıştı!**`) .addField("Mesajı:",msg)
         client.channels.cache.get(y).send(embed)
            }              
          } catch(err) {
            console.log(err);
...
client.on("messageUpdate", async (old, nev) => {
   ...
   if (i) {
      ...
      if (nev.member.hasPermission("BAN_MEMBERS")) return ;
       //if (ayarlar.gelistiriciler.includes(nev.author.id)) return ;
 const embed = new Discord.MessageEmbed() .setColor('#05c7ff') .setDescription(` ${nev.author} , **Mesajını editleyerek küfür etmeye çalıştı!**`)
            .addField("Mesajı:",nev)
        
            nev.delete();
            const embeds = new Discord.MessageEmbed() .setColor('#05c7ff') .setDescription(` ${nev.author} , **Mesajı editleyerek küfür etmene izin veremem!**`) 
          client.channels.cache.get(y).send(embed)
            nev.channel.send(embeds).then(msg => msg.delete({timeout:5000}));
          ...
          if (i) {
              if (küfür.some(word => msg.content.toLowerCase().includes(word))) {
                try {
                 ...
                 //  if (!ayarlar.gelistiriciler.includes(msg.author.id)) return ;
     msg.delete({timeout:750});
                    const embeds = new Discord.MessageEmbed() .setColor('#05c7ff') .setDescription(` <@${msg.author.id}> , **Bu sunucuda küfür yasak!**`)
      msg.channel.send(embeds).then(msg => msg.delete({timeout: 5000}));
                const embed = new Discord.MessageEmbed() .setColor('#05c7ff') .setDescription(` ${msg.author} , **Küfür etmeye çalıştı!**`) .addField("Mesajı:",msg)
               client.channels.cache.get(y).send(embed)
                  }              
                } catch(err) {
                  console.log(err);
